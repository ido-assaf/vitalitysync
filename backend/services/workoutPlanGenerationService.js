...
const { buildFitnessCoachContext } = require("./aiSpecialistContextService");
const { buildSpecialistRules } = require("./aiSpecialistRuleService");
const { retrieveFitnessKnowledge } = require("./aiSpecialistKnowledgeService");
const { buildFitnessWeeklyReview } = require("./aiSpecialistWeeklyReviewService");
const { normalizeWeeklyCheckInHistory } = require("./aiSpecialistWeeklyCheckInService");
const {
  boundedScoreDelta,
  buildFitnessPlanningModifiers,
  emptyFitnessPlanningModifiers
} = require("./aiSpecialistPlanningModifierService");
const { validateFitnessPlanQuality } = require("./aiSpecialistPlanValidatorService");
const {
  equipmentMatches,
  isExerciseAllowed,
...
// Deterministic workout plan generation engine plus the weekly fitness review
// orchestration. Owns the day-split templates, slot definitions, exercise
// scoring/selection waterfall, set/rep prescriptions, suggested-plan notes, and
// the suggested-plan upsert/backfill. The workoutPlansController keeps HTTP
// shaping only. Suggested plans are identified by SUGGESTED_PLAN_PREFIX at the
// start of the notes column; that note composition is a load-bearing contract.
...
// Shared assembly for plan generation and the weekly review: resolve the acting
// coach, then build the trainee context, expert rules, and knowledge items.
async function buildFitnessSpecialistInputs(userId, profile) {
  ...
  return { aiCoach, specialistContext, expertRules, professionalKnowledge };
}
...
async function upsertSuggestedWorkoutPlanForUser(userId) {
  ...
  const { aiCoach, specialistContext, expertRules, professionalKnowledge } =
    await buildFitnessSpecialistInputs(userId, profile);
...
async function buildWeeklyFitnessReviewForUser(userId, profile, options = {}) {
  const { specialistContext, expertRules, professionalKnowledge } =
    await buildFitnessSpecialistInputs(userId, profile);
